...
start "Inventory Service" cmd /k "set PYTHONPATH=. && .venv\Scripts\python.exe -m uvicorn app.inventory-service.main:app --port 8002"
timeout /t 2 >nul

echo [2/3] Starting Payment Service (Port 8004)...
start "Payment Service" cmd /k "set PYTHONPATH=. && .venv\Scripts\python.exe -m uvicorn app.payment-service.main:app --port 8004"
timeout /t 2 >nul

echo [3/3] Starting API Gateway (Port 8003)...
start "API Gateway" cmd /k "set PYTHONPATH=. && .venv\Scripts\python.exe -m uvicorn app.api-gateway.main:app --port 8003" 
